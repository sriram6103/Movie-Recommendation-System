...
emotion_to_genre = {
    ...
    'Fear': ['Horror'],
    'Disgust': ['Drama'],
    'Neutral': ['Comedy','Drama','Action','Mystery','Horror']
}
...
def get_movie_recommendations(emotion, num_recommendations):
    ...
    if 'Any' in genres:
        filtered_movies = smd  # Select all movies if the emotion is "Neutral"
    else:
...
def display_recommendations(movies):
    ...
    # Title label
    label = tk.Label(window, text="Recommended Movies", font=("Helvetica", 18, 'bold'), bg='#f0f0f0')
    label.pack(pady=10)
